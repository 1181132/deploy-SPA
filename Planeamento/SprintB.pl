...
%armazem principial matosinhos
armazemPrincipal(5).

%camiao(nome, capacidade carga, carga total baterias, automomia max, tempo carregamento)
camiao(eTruck01,4300,80,100,60).

%peso camiao
pesoCamiao(7500).

%metendo entregas por parametro saber em que armazens tem que parar
armazemVisitar([], []).

armazemVisitar([Head|Tail], ListStopsID):-
    armazemVisitar(Tail, TempList),
    entrega(Head, _, _, WarehouseID, _, _),
    append([WarehouseID], TempList, ListStopsID).


%Peso por cada armazem que visita

pesoPorArmazem(ListWarehouse, ListWeigths):-
    pesoPorParagem(ListWarehouse, ListWeigths, _).


%Peso somar os pesos de todas as paragens
pesoPorParagem([], [0], 0).

pesoPorParagem([Head2|Tail2], [Acc|Tail3], Acc):-
    pesoPorParagem(Tail2, Tail3, TempVar),
    entrega(_, _, Weight, Head2, _, _),
    Acc is TempVar + Weight.

%camiao(nome, capacidade carga, carga total baterias, automomia max, tempo carregamento)
%Calcular peso total do camiao com os pesos mais peso camiao entrando por parametro a lista de pesos das entregas
calculaPesoTotal([], []).

calculaPesoTotal([Head1|Tail1], [TotalWeight|Tail2]):-
    calculaPesoTotal(Tail1, Tail2),
    pesoCamiao(Tare),
    TotalWeight is Tare + Head1.

%retorna energia minima do camiao
energiaMinima(TruckName, Result):-
    camiao(TruckName, _,MaxCap, _, _),
    TempVar is MaxCap * 20,
    Result is TempVar / 80. 

%retorna energia maxima do camiao
energiaMaxima(TruckName, Result):-
    camiao(TruckName,_, MaxCap, _, _),
    Result is MaxCap.

%calcular tempo de carga
calcularTempoDeRecarga(TruckName, MaxEnergy, MinEnergy, RechargeEnergy, Result):-
    camiao(TruckName, _, _, _, RechargeTime),
...
%TruckMaxWeight peso camiao cheio
%Weight peso que o camiao tem no momento
calcularEnergiaConsumida(A, B, TruckMaxWeight, Weight, EnergyConsumed):-
    dadosCam_t_e_ta(_,A,B,_,Energy,_),
...
calcularPesoMaximoCamiao(TruckName, Result):-
    camiao(TruckName, Tare, _, _, _),
    pesoCamiao(Weight),
    Result is Tare + Weight.

%calculo tempo entrega
%TruckMaxWeight peso camiao cheio
%Weight peso que o camiao tem no momento
calculaTempoViagem(A, B, TruckMaxWeight, Weight, TimeTravel):-
    dadosCam_t_e_ta(_,A,B,Time,_,_),
    TempVar is Weight * Time,
    TimeTravel is TempVar / TruckMaxWeight.


%entrega(<idEntrega>,<data>,<massaEntrefa>,<armazemEntrega>,<tempoColoc>,<tempoRet>)
%tempo retirar entrega metendo 0 no fim por causa da ultima viagem quando chega armazem principal

tempoRetirarEntrega([], [0]).

tempoRetirarEntrega([Head|Tail], ListTime):-
    tempoRetirarEntrega(Tail,Temp),
    entrega(Head,_,_,_,_,Time),
...
completarTodasRotas([Head1|Tail1], [Head2|Tail2]):-
    completarTodasRotas(Tail1, Tail2),
    armazemPrincipal(AP),
    append([AP], Head1, TempList),
    append(TempList, [AP], Head2).*/

completarTodasRotas(List, AllList):-
    armazemPrincipal(AP),
    append([AP|List],[AP],AllList).



%--------------------------------------------------------
% Tempo Colocacao Nao é considerado
%adicionar Tempo de colocaçao
adicionarTempoColocacao(_, [], []).

adicionarTempoColocacao(Warehouses, [Head1|Tail1], [Head2|Tail2]):-
    calcularTempoColocacao(Warehouses, LoadingTime),
    Head2 is Head1 + LoadingTime,
    adicionarTempoColocacao(Warehouses, Tail1, Tail2).

%entrega(<idEntrega>,<data>,<massaEntrefa>,<armazemEntrega>,<tempoColoc>,<tempoRet>)
%calcular Tempo de colocacao
calcularTempoColocacao([], 0).

calcularTempoColocacao([Head|Tail], Result):-
    calcularTempoColocacao(Tail, TempVar),
    entrega(_, _, _, Head, PlacementTime, _),
...
tempoDeViagem(TruckName, CurrCap, [Head1A, Head1B|Tail1], [Head2|Tail2], [Head3|Tail3], TravelTime):-
    ...
    calcularEnergiaConsumida(Head1A, Head1B, TruckMaxWeight, Head2, EnergyConsumed),
    EnergyLeft is CurrCap - EnergyConsumed,
    energiaMinima(TruckName, MinEnergy),
    EnergyLeft >= MinEnergy,
    calculaTempoViagem(Head1A, Head1B, TruckMaxWeight, Head2, ElapsedTime),
    tempoDeViagem(TruckName, EnergyLeft, [Head1B|Tail1], Tail2, Tail3, TempVar1),
...
tempoDeViagem(TruckName, CurrCap, [Head1A, Head1B|Tail1], [Head2|Tail2], [Head3|Tail3], TravelTime):-
    ...
    energiaMaxima(TruckName, MaxEnergy),
    EnergyConsumed =< MaxEnergy,
    TempVar1 is EnergyConsumed + MinEnergy,
    RechargeEnergy is TempVar1 - CurrCap,
    calcularTempoDeRecarga(TruckName, MaxEnergy, MinEnergy, RechargeEnergy, RechargeTime),
    TempVar2 is CurrCap + RechargeEnergy,
    EnergyLeft2 is TempVar2 - EnergyConsumed,
    calculaTempoViagem(Head1A, Head1B, TruckMaxWeight, Head2, ElapsedTime),
    tempoDeViagem(TruckName, EnergyLeft2, [Head1B|Tail1], Tail2, Tail3, TempVar3),
...
tempoDeTodasRotas(TruckName, ListDeliveriesID, RouteList, TravelTimeList):-
    ...
    pesoPorArmazem(ListStopsID, ListWeigths),
    calculaPesoTotal(ListWeigths, ListTotalWeight),
    findall(PermListStopsID, permutation(ListStopsID, PermListStopsID), AllPermStopsID),
    completarTodasRotas(AllPermStopsID, TempList1),
    camiao(TruckName, _, MaxCap, _, _),
    tempoRetirarEntrega(ListDeliveriesID, ListUnloadingTime),
    tempoCadaRota(TruckName, MaxCap, TempList1, ListTotalWeight, ListUnloadingTime, TempList2, RouteList).
    adicionarTempoColocacao(ListDeliveriesID, TempList2, TravelTimeList).


%tempo de cada rota
tempoCadaRota(_, _, [], _, _, [], []).

tempoCadaRota(TruckName, CurrCap, [Head1|Tail1], ListTotalWeight,  ListUnloadingTime, [Head2|Tail2], [Head1|Tail3]):-
    tempoDeViagem(TruckName, CurrCap, Head1, ListTotalWeight, ListUnloadingTime, Head2),
    !,
    tempoCadaRota(TruckName, CurrCap, Tail1, ListTotalWeight, ListUnloadingTime, Tail2, Tail3).

tempoCadaRota(TruckName, CurrCap, [_|Tail], ListTotalWeight,  ListUnloadingTime, TravelTimeList, PossibleRoutes):-
    tempoCadaRota(TruckName, CurrCap, Tail, ListTotalWeight, ListUnloadingTime, TravelTimeList, PossibleRoutes).


%-------

% calcula tempo de viagem introduzir lista de entregas
calcularTempoDeViagem(ListDelivery,Time):-
armazemVisitar(ListDelivery,ListWarehouse),
...
tempoRetirarEntrega(ListDelivery,UnloadingTime),
